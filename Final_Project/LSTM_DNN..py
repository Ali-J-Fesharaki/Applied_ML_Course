...
def initialize_params(input_size, hidden_size, output_size, bc, bh):
    ...
    weights = [np.random.rand(weightsSizes[0][i], weightsSizes[1][i]) * 0.05 for i in range(L-1)]
...
c_M = np.random.rand(hidden_size_M, num_steps+1)
h_M = np.random.rand(hidden_size_M, num_steps+1)
c_V = np.random.rand(hidden_size_V, num_steps+1)
h_V = np.random.rand(hidden_size_V, num_steps+1)
c_G = np.random.rand(hidden_size_G, num_steps+1)
h_G = np.random.rand(hidden_size_G, num_steps+1)
...
for k in range(1, num_steps+1):
    ...
    dc_M, dh_M, lstm_output_M, Theta_hat_prim_M = lstm_cell(x0_M, c_prev_M, h_prev_M, params_M)
    dc_V, dh_V, lstm_output_V, Theta_hat_prim_V = lstm_cell(x0_V, c_prev_V, h_prev_V, params_V)
    dc_G, dh_G, lstm_output_G, Theta_hat_prim_G = lstm_cell(x0_G, c_prev_G, h_prev_G, params_G)
    ...
    hat_Xi_V = dnn_output_V + lstm_output_V
    hat_Xi_G = dnn_output_G + lstm_output_G
    hat_Xi_F = dnn_output_F + lstm_output_F
    hat_Xi_M = dnn_output_M + lstm_output_M
    ...
    # Adaptation Law
    Xi_M_prim = np.hstack((Theta_hat_prim_M, Phi_hat_prim_M))
    Xi_V_prim = np.hstack((Theta_hat_prim_V, Phi_hat_prim_V))
    Xi_G_prim = np.hstack((Theta_hat_prim_G, Phi_hat_prim_G))
    Xi_F_prim = np.hstack((Theta_hat_prim_F, Phi_hat_prim_F))
    ...
    params_M['weights'] = vec2param_dnn(Z_M[len(vv_M):], params_M)
    params_G['weights'] = vec2param_dnn(Z_G[len(vv_G):], params_G)
    params_V['weights'] = vec2param_dnn(Z_V[len(vv_V):], params_V)
    params_F['weights'] = vec2param_dnn(Z_F[len(vv_F):], params_F)

    Wc, Wp, Wf, Wo, Wh = vec2param_lstm(Z_M[:len(vv_M)], params_M)
    params_M['Wc'], params_M['Wp'], params_M['Wf'], params_M['Wo'], params_M['Wh'] = Wc, Wp, Wf, Wo, Wh

    Wc, Wp, Wf, Wo, Wh = vec2param_lstm(Z_V[:len(vv_V)], params_V)
    params_V['Wc'], params_V['Wp'], params_V['Wf'], params_V['Wo'], params_V['Wh'] = Wc, Wp, Wf, Wo, Wh

    Wc, Wp, Wf, Wo, Wh = vec2param_lstm(Z_G[:len(vv_G)], params_G)
    params_G['Wc'], params_G['Wp'], params_G['Wf'], params_G['Wo'], params_G['Wh'] = Wc, Wp, Wf, Wo, Wh

    Wc, Wp, Wf, Wo, Wh = vec2param_lstm(Z_F[:len(vv_F)], params_F)
    ...
    # Update states
    c_M[:, k] = dc_M * dt + c_M[:, k-1]
    h_M[:, k] = dh_M * dt + h_M[:, k-1]
    c_V[:, k] = dc_V * dt + c_V[:, k-1]
    h_V[:, k] = dh_V * dt + h_V[:, k-1]
    c_G[:, k] = dc_G * dt + c_G[:, k-1]
    h_G[:, k] = dh_G * dt + h_G[:, k-1]
    c_F[:, k] = dc_F * dt + c_F[:, k-1]
    h_F[:, k] = dh_F * dt + h_F[:, k-1]
    ...
    qd_vec[k - 1, :] = qd
    u_vec[k - 1, :] = tau
    output_M_List[k-1,:]=lstm_output_M



# Plotting the results
# Plot Cell State Evolution
plt.figure(figsize=(10, 8))
plt.subplot(3, 1, 1)
plt.plot(np.arange(num_steps +1) * dt, c_M.T)
plt.xlabel('Time (s)')
...
plt.subplot(3, 1, 2)
plt.plot(np.arange(num_steps+1 ) * dt, h_M.T)
plt.xlabel('Time (s)')
plt.ylabel('Hidden State')
plt.title('Hidden State Evolution')

# Plot LSTM Output Evolution
plt.subplot(3, 1, 3)
plt.plot(np.arange(num_steps ) * dt, output_M_List)
plt.xlabel('Time (s)')
plt.ylabel('Output')
plt.title('LSTM Output Evolution')
